...
export default function LavaLampBackground() {
  ...
  useEffect(() => {
    ...
    blobs.forEach((blob, i) => {
      ...
      let scale = 1;
      let scaleDir = 1;
      let scaleSpeed = 0.0004 + Math.random() * 0.0006;
      let wobbleAngle = Math.random() * Math.PI * 2;
      let wobbleSpeed = 0.004 + Math.random() * 0.005;
      let borderRadiusPhase = Math.random() * Math.PI * 2;
      let frameCount = 0;

      const animate = () => {
        frameCount++;
        // Only update every 2 frames to halve CPU usage
        if (frameCount % 2 === 0) {
          x += vx;
          y += vy;

          wobbleAngle += wobbleSpeed;
          vx += Math.sin(wobbleAngle) * 0.004;
          vy += Math.cos(wobbleAngle * 0.7) * 0.004;
          ...
          if (scale > 1.18) scaleDir = -1;
          if (scale < 0.85) scaleDir = 1;

          borderRadiusPhase += 0.006;
          const r1 = 40 + Math.sin(borderRadiusPhase) * 12;
          const r2 = 40 + Math.cos(borderRadiusPhase * 0.8) * 14;
          const r3 = 40 + Math.sin(borderRadiusPhase * 1.3) * 10;
          const r4 = 40 + Math.cos(borderRadiusPhase * 0.6) * 16;
